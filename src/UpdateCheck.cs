...
internal static class UpdateCheck
{
    private const string GitHubLatestReleaseApi =
        "https://api.github.com/repos/vgArchives/ReStory-ReColor/releases/latest";

    private const string NexusModPage = "https://www.nexusmods.com/restorychillelectronicrepairs";
    ...
    internal static IEnumerator Run(string currentVersion)
    {
        ...
        request.SetRequestHeader("User-Agent", ReColorPlugin.PluginName);
    ...
    private static bool TryGetNewer(string releaseJson, string currentVersion, out Version latestVersion)
    {
        ...
        bool isReadableVersion = Version.TryParse(latestReleaseTag?.TrimStart('v', 'V'), out latestVersion);

        if (!isReadableVersion)
            return false;

        return latestVersion > new Version(currentVersion);
    }
